                    ...
                    if(guide.swimmingStyle.includes(student.swimmingStyle))
                    {
                        ...
                        if(!classStartTimeArray.includes(startTimeString))
                        {
                            if(!((diffHours <= 1) && ((Math.abs(diffMinutes) === 45) || (Math.abs(diffMinutes) === 0))))
                            {
                                ...
                                {
                                   // let endTime = setTime(guide.startTime.getHours(), guide.startTime.getMinutes() + 45).toLocaleTimeString([], { hour: '2-digit', minute: "2-digit", hour12: false});
                                    let newStartTime = new Date();///chhhhh
                                    newStartTime = setTime(startHour , startMinutes);///chhhhh
                                    let newEndTime = setTime(guide.startTime.getHours(), guide.startTime.getMinutes() + 45)///chhhhh
                                    ...
                                    for(let i = 0; i < collectiveClasses.length; i++)
                                    {
                                        if(collectiveClasses[i].style === student.swimmingStyle)
                                        {
                                            ...
                                            if(collectiveClasses[i].count === 2 && collectiveClasses[i].guideName === guide.firstName)
                                            {
                                                ...
                                                //collectiveClasses[i].startCollectiveTime = guide.startTime.toLocaleTimeString([], { hour: '2-digit', minute: "2-digit", hour12: false});
                                                //collectiveClasses[i].endCollectiveTime = setTime(guide.startTime.getHours(), guide.startTime.getMinutes() + 60).toLocaleTimeString([], { hour: '2-digit', minute: "2-digit", hour12: false});
                                ...
                                else if((Math.abs(diffMinutes) <= 45) && student.lessonType === 'both')
                                {
                                    ...
                                    classes.push({
                                        ...
                                        endTime: newEndTime,
                                        color:'#ff7f50',
                                        description:`During this time there will be a swimming lesson between ${guide.firstName} and ${students.firstName}. `
                                    });
        ...
        for(let i = 0; i < collectiveClasses.length; i++)
        {
           if(collectiveClasses[i].count >= 2)
           {
                classes.push({
                  ...
                  color: '#5f9ea0',
                  description:`During this time there will be a collective swimming lesson with ${collectiveClasses[i].guideName} at ${collectiveClasses[i].startCollectiveTime} ${collectiveClasses[i].style} ${collectiveClasses[i].studentsCollective.map((student) => 
                      student.firstName
                  ) }. `
                });
